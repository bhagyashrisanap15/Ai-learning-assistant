...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return <>
    ...
    <aside
      className={`fixed top-0 left-0 h-full w-64 ■bg-white/90 backdrop-blur-lg border-r border-neutral-200 z-50 md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out${
    ...
    >
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 ${
                isActive
                  ? 'bg-linear-to-r ■from-emerald-500 ■to-teal-500 ■text-white shadow-emerald-500/25'
      ...
      <div className="px-3 py-4 border-t border-slate-200/60">
        <button
          ...
          className="group flex otems-center gap-3 w-full px-4 py-2.5 text-sm font-semibol text-slate-700 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all duration-200 "
        >
          <LogOut
            size={18}
            strokeWidth={2.5}
